...
// AI Assistant Summary Component - Uses aynstyn feedback from assessment response
function AIAssistantSummary({ aynstynSummary, fallbackFeedback }: { aynstynSummary: any, fallbackFeedback: string }) {
  // If aynstyn summary is available, use it directly from the assessment response
  const assistantData = aynstynSummary ? { success: true, assistantSummary: aynstynSummary } : null;
  const isLoading = false; // No loading since data comes from assessment response
  const error = !aynstynSummary;
    ...
    <TransitionSection className="mb-8" delay={0.6} direction="up">
      ...
      {isLoading && (
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-6">
          <div className="flex items-center justify-center py-8">
            ...
            <span className="ml-2 text-gray-600">aynstyn is analyzing your assessment...</span>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="bg-primary-50 border border-primary-100 rounded-lg p-6">
          <div className="prose prose-sm max-w-none text-gray-700">
            <p className="whitespace-pre-line">{fallbackFeedback}</p>
          </div>
        </div>
      )}

      {assistantData && assistantData.success && (
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-6">
          <div className="prose prose-lg max-w-none text-gray-700">
            <p className="whitespace-pre-line leading-relaxed text-lg">
              {assistantData.assistantSummary.enhancedFeedback || fallbackFeedback}
            </p>
          </div>
        </div>
      )}
      ...
      {assistantData && (
        <div className="space-y-6">
          {/* Learning Path */}
          {assistantData.assistantSummary.learningPath.length > 0 && (
            <div className="bg-white rounded-lg p-5 border border-blue-100">
              ...
              <div className="space-y-3">
                {assistantData.assistantSummary.learningPath.map((step: string, index: number) => (
          ...
          {/* Next Steps */}
          {assistantData.assistantSummary.nextSteps.length > 0 && (
            <div className="bg-white rounded-lg p-5 border border-green-100">
              ...
              <div className="space-y-2">
                {assistantData.assistantSummary.nextSteps.map((step: string, index: number) => (
          ...
          {assistantData.assistantSummary.resourceRecommendations.length > 0 && (
            <div className="bg-white rounded-lg p-5 border border-orange-100">
              <h4 className="font-semibold text-gray-900 mb-3 flex items-center">
                <BookMarked className="mr-2 h-4 w-4 text-orange-600" />
                Recommended Resources
              </h4>
              <div className="space-y-2">
                {assistantData.assistantSummary.resourceRecommendations.map((resource: string, index: number) => (
                  <div key={index} className="flex items-center">
...
export default function ResultsSection({ result }: ResultsSectionProps) {
  ...
  useEffect(() => {
    ...
    if (circleRef.current) {
      ...
      setTimeout(() => {
        if (circleRef.current) {
          const scorePercentage = result.score > 1 ? result.score : result.score * 100;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row mb-8 gap-6">
        ...
        <AnimatedSection className="flex-1 bg-gray-50 rounded-xl p-6 flex flex-col items-center" delay={0.1}>
          ...
          <p className="text-center text-sm text-gray-600 mb-2">
            {result.score >= 80
              ? "Well-articulated knowledge with logical flow and real understanding."
              : result.score >= 60
                ? "Covers key ideas using accurate terminology, but may miss some context."
                : result.score >= 40
                  ? "Shows basic awareness of the subject, but lacks depth or structure."
                  : result.score >= 20
                    ? "Barely relevant content with limited understanding shown."
                    : "Minimal engagement with the subject matter."}
          </p>

          <div className="text-center text-xs text-gray-500 border-t border-gray-200 pt-3 mt-3 px-4">
            <p className="font-medium mb-1">How we measure your understanding:</p>
            <p>
              Our algorithm analyzes topic coverage, factual accuracy, and depth of 
              insight to provide a comprehensive assessment of your knowledge.
            </p>
          </div>
        </AnimatedSection>
          ...
          <div className="space-y-5">
            {result.topicCoverage && result.topicCoverage.length > 0 ? (
              ...
              <div className="text-center py-8 text-gray-500">
                <p className="text-sm">
                  Topic coverage percentages were not generated for this assessment.
                </p>
                <p className="text-xs mt-2">
                  Your assessment focused on the covered and missing topics shown below.
                </p>
              </div>
            )}
          </div>
      ...
      {/* Enhanced AI Assistant Summary - Replaces Personalized Feedback */}
      <AIAssistantSummary aynstynSummary={result.aynstynSummary} fallbackFeedback={result.feedback} />
      

      {/* Learning Task Summary */}
      <TransitionSection delay={0.65} direction="up">
        <div className="w-full rounded-lg border border-blue-200 bg-blue-50 p-6 mb-6">
          <div className="flex items-center mb-4">
            <div className="flex-shrink-0 bg-blue-100 rounded-full p-2">
              <BookOpen className="h-5 w-5 text-blue-600" />
            </div>
            <h3 className="text-lg font-semibold text-blue-900 ml-3">
              Your Next Learning Tasks
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-lg p-4 border border-blue-100">
              <div className="mb-3">
                <h4 className="text-lg font-semibold text-orange-600 mb-2 flex items-center">
                  <span className="bg-orange-100 rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold mr-2">
                    {result.missingTopics.length}
                  </span>
                  Topics to Explore
                </h4>
            ...
            <div className="bg-white rounded-lg p-4 border border-blue-100">
              <div className="mb-3">
                <h4 className="text-lg font-semibold text-blue-600 mb-2 flex items-center">
                  <span className="bg-blue-100 rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold mr-2">
                    {result.missingTopics.reduce((total, topic) => 
                      total + (topic.keyPoints ? topic.keyPoints.length : 0), 0
                    )}
                  </span>
                  Key Concepts to Learn
                </h4>
                <ul className="space-y-1 max-h-32 overflow-y-auto">
                  {result.missingTopics.map((topic, topicIndex) => 
                    topic.keyPoints ? topic.keyPoints.map((keyPoint, pointIndex) => (
                      <li key={`${topicIndex}-${pointIndex}`} className="text-sm text-gray-700 flex items-start">
          ...
          <CollapsibleContent>
            <div className="px-4 pb-4">
              <div className="bg-white border border-gray-100 rounded p-3 text-sm text-gray-600 max-h-60 overflow-y-auto whitespace-pre-line">
                {result.inputType === "audio" 
                  ? (result.transcribedText || "No transcription available.") 
                  : (result.input || result.userInput || "No input text available.")}
                {result.inputType === "audio" && (
                  <div className="mt-3 pt-2 border-t border-gray-100">
                    <p className="text-xs text-blue-600 font-medium mb-1">Audio Input</p>
                    <p className="text-xs text-gray-500">
                      This analysis was performed on your voice recording, transcribed using OpenAI's Whisper speech recognition technology.
                    </p>
                  </div>
                )}
              </div>
